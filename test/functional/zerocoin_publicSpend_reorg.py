...
class ZerocoinPublicSpendReorg(PIVX_FakeStakeTest):
    ...
    def run_test(self):
        ...
        # 7) Now create block_D which tries to spend tx_B again on the (new) main chain
        # (this block will be rejected)
        block_count += 1
        self.log.info("*** block_D ***")
        self.log.info("Creating block_D [%d] trying to double spend tx_B" % (block_count + 1))
        block_D = self.new_block(block_count, block_C, checkpoint, tx_B)

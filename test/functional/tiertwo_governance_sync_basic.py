...
class MasternodeGovernanceBasicTest(PivxTestFramework):
    ...
    def _initialize_chain_clean(self):
        for i in range(self.num_nodes):
            extra_args = {}
            if (i == self.remoteOnePos):
                remotePort = p2p_port(self.remoteOnePos)
                masternodePrivKey = self.mnOnePrivkey
                extra_args["listen"] = "1"
                extra_args["externalip"] = "127.0.0.1"
                extra_args["masternodeaddr"] = "127.0.0.1:"+str(remotePort)
                extra_args["masternodeprivkey"] = str(masternodePrivKey)

            if (i == self.remoteTwoPos):
                remotePort = p2p_port(self.remoteTwoPos)
                masternodePrivKey = self.mnTwoPrivkey
                extra_args["listen"] = "1"
                extra_args["externalip"] = "127.0.0.1"
                extra_args["masternodeaddr"] = "127.0.0.1:"+str(remotePort)
                extra_args["masternodeprivkey"] = str(masternodePrivKey)

            initialize_datadir(self.options.tmpdir, i, extra_args)
    ...
    def run_test(self):
        ...
        remoteOne.initmasternode(self.mnOnePrivkey)
        remoteTwo.initmasternode(self.mnTwoPrivkey)

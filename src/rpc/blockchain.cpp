...
UniValue getblockheader(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  {\n"
            "     \"chainValue\":        (bool) Total value held by the Sapling circuit up to and including this block\n"
            "     \"valueDelta\":        (bool) Change in value held by the Sapling circuit over this block\n"
            "  }\n"
            "}"
            "}\n"
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  {\n"
            "     \"chainValue\":        (bool) Total value held by the Sapling circuit up to and including the chain tip\n"
            "     \"valueDelta\":        (bool) Change in value held by the Sapling circuit over the chain tip block\n"
            "  }\n"
